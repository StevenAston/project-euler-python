# -*- coding: utf-8 -*-
"""
Created on Sat Jun 10 04:17:41 2017

@author: Steven
"""

grid = ""

matrix = [[0] * 20 for i in range(20)]
x = 0
y = 0
product = 1

for i in range(0,len(grid)-1,2):
    matrix[x][y] = int(grid[i:i+2])
    y += 1
    if y >= 20:
        y = 0
        x += 1

# Search Vertical
for y in range(0,1):
    for x in range(0,17):
        if (matrix[x][y] * matrix[x+1][y] * matrix[x+2][y] * matrix[x+3][y]) > product:
            product = (matrix[x][y] * matrix[x+1][y] * matrix[x+2][y] * matrix[x+3][y])
            print("Vertical - %d * %d * %d * %d = %d" % (matrix[x][y], matrix[x+1][y], matrix[x+2][y], matrix[x+3][y], product))
   
# Search Horizontal
for x in range(0,20):
    for y in range(0,17):
        if (matrix[x][y] * matrix[x][y+1] * matrix[x][y+2] * matrix[x][y+3]) > product:
            product = (matrix[x][y] * matrix[x][y+1] * matrix[x][y+2] * matrix[x][y+3])
            print("Horizontal - %d * %d * %d * %d = %d" % (matrix[x][y],matrix[x][y+1],matrix[x][y+2],matrix[x][y+3], product))

# Search Diagonal Right
for x in range(0,17):
    for y in range(0,17):
        if (matrix[x][y] * matrix[x+1][y+1] * matrix[x+2][y+2] * matrix[x+3][y+3]) > product:
            product = (matrix[x][y] * matrix[x+1][y+1] * matrix[x+2][y+2] * matrix[x+3][y+3])
            print("Diagonal Right - %d * %d * %d * %d = %d" % (matrix[x][y], matrix[x+1][y+1], matrix[x+2][y+2], matrix[x+3][y+3], product))

# Search Diagonal Left
for y in range(3,20):
    for x in range(0,17):
        if (matrix[x][y] * matrix[x+1][y-1] * matrix[x+2][y-2] * matrix[x+3][y-3]) > product:
            product = (matrix[x][y] * matrix[x+1][y-1] * matrix[x+2][y-2] * matrix[x+3][y-3])
            print("Diagonal Left - %d * %d * %d * %d = %d" % (matrix[x][y], matrix[x+1][y-1], matrix[x+2][y-2], matrix[x+3][y-3], product))
           
#print(product)
#print(matrix[1][2])